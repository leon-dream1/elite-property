...
const RequestedPropertyDataRow = ({ property, handleAcceptOrRejectPropertyRequest }) => {
  return (
    <tr>
      ...
      {property?.status === "accepted" || property?.status === "rejected" || (
        <td className="px-3 py-3 border-b border-gray-200 bg-white text-sm">
          <button
            onClick={() => handleAcceptOrRejectPropertyRequest(property?._id, "accepted")}
      ...
      {property?.status === "accepted" || property?.status === "rejected" || (
        <td className="px-3 py-3 border-b border-gray-200 bg-white text-sm">
          <button
            onClick={() => handleAcceptOrRejectPropertyRequest(property?._id, "rejected")}
            className="relative cursor-pointer inline-block px-3 py-1 font-semibold text-green-900 leading-tight"
          >
            <span
              aria-hidden="true"
